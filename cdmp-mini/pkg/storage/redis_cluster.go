...
func (r *RedisCluster) Eval(script string, keys []string, args []interface{}) (interface{}, error) {
	...
	switch v := client.(type) {
	case *redis.ClusterClient:
		return v.Eval(script, keys, args).Result()
	case *redis.Client:
		return v.Eval(script, keys, args).Result()
	default:
...
// EvalSha 执行Lua脚本的SHA1哈希
func (r *RedisCluster) EvalSha(sha1 string, keys []string, args []interface{}) (interface{}, error) {
	if err := r.up(); err != nil {
		return nil, err
	}
	...
	switch v := client.(type) {
	case *redis.ClusterClient:
		return v.EvalSha(sha1, keys, args).Result()
	case *redis.Client:
		return v.EvalSha(sha1, keys, args).Result()
	default:
		return nil, errors.New("unsupported redis client type")
...
func (r *RedisCluster) ScriptLoad(script string) (string, error) {
	...
	switch v := client.(type) {
	case *redis.ClusterClient:
		return v.ScriptLoad(script).Result()
	case *redis.Client:
		return v.ScriptLoad(script).Result()
	default:
...
// SetNX 设置键值，仅当键不存在时（原子操作）
func (r *RedisCluster) SetNX(keyName string, value interface{}, expiration time.Duration) (bool, error) {
	if err := r.up(); err != nil {
		return false, err
	}

	result, err := r.singleton().SetNX(keyName, value, expiration).Result()
	if err != nil {
