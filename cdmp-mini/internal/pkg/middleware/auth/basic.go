...
type BasicStrategy struct {
	compare func(c context.Context, username, password string) bool
}

var _ middleware.AuthStrategy = &BasicStrategy{}

func NewBasicStrategy(compare func(c context.Context, username, password string) bool) *BasicStrategy {
...
func (b *BasicStrategy) AuthFunc() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if len(auth) != 2 || auth[0] != "Basic" {
			core.WriteResponse(c, errors.WithCode(code.ErrSignatureInvalid, "解析头失败"), nil)
			c.Abort()
			return
		}
